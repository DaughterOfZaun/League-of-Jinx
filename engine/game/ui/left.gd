...
func _process(delta: float) -> void:
	attack_damage_label.text = str(roundi(champion.stats_perm.get_attack_damage()))
	magical_damage_label.text = str(roundi(champion.stats_perm.get_magic_damage()))
	attack_speed_label.text = str(roundf(champion.stats_perm.get_attack_speed() * 1000.0) / 1000.0)
	movement_speed_label.text = str(roundi(champion.stats_perm.get_movement_speed()))
	armor_label.text = str(roundi(champion.stats_perm.get_armor()))
	spell_block_label.text = str(roundi(champion.stats_perm.get_spell_block()))
	level_label.text = str(champion.stats_perm.level)
