...
const effect_0: Array[float] = [ 30, 60, 90, 120, 150 ]
var spell_vars_ready: int #TODO: Verify
func on_missile_update(missile: Missile) -> void:
	spell_vars_ready += 1
	if spell_vars_ready < 3: return

	var level := host.spells.w.level
	for unit: Unit in API.get_closest_visible_units_in_area(host, missile.position_3d, 650, Enums.SpellFlags.AFFECT_ENEMY_HEROES, 1, null, true):
	...
	for unit: Unit in API.get_closest_visible_units_in_area(host, missile.position_3d, 650, Enums.SpellFlags.AFFECT_ENEMY_AND_NEUTRAL_MINIONS_AND_HEROES, 1, null, true):
		(host.spells.extra[3] as AhriFoxFireMissileTwoSpell).cast(unit, Vector3.INF, Vector3.INF, level, true, true, false, true, false, true, missile.position_3d)
		host.buffs.remove_stacks(AhriFoxFireMissileBuff, 1, host)
		missile.destroy()
		
		#level = host.spells.w.level
		var base_damage := self.effect_0[level - 1]
